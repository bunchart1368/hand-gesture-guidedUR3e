        ...
        try:
            ...
            if isinstance(position, int):
                ...
                if total_force > 30: 
                    ...
                    emergency_stop = True
                    return
                robot_position = robot_position_list[-1]
                accumulated_pose = accumulated_pose_list[-1]
                previous_command = previous_command_list[-1]
                robot_position = compute_target_pose(robot_position, position, command)
                print("Target Pose: ", robot_position)
                ...
                robot_position_list.clear()
                robot_position_list.append(robot_position)
                accumulated_pose_list.clear()
                accumulated_pose_list.append(accumulated_pose)
                previous_command_list.clear()
...
def start_server():
    ...
    global command_collection, robot_position_list, accumulated_pose_list, previous_command_list
    command_collection = list()
    robot_position_list = list()
    accumulated_pose_list = list()
    previous_command_list = list()
    robot_position_list.append(robot_position)
    accumulated_pose_list.append(accumulated_pose)
    previous_command_list.append(previous_command)
